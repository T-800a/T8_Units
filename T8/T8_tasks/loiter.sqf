/*
 =======================================================================================================================
	...
	Example(s):
	tmp = [ group this, ( getPos this ) ] execVM "loiter.sqf";
	tmp = [ group this, ( getPos this ), 100 ] execVM "loiter.sqf";
...
if ( T8U_var_DEBUG ) then { [ ">> %1 >>>>>>>>>> loiter.sqf >> wait until unit is in Range of LoiterPos >> %2", time, _group ] call BIS_fnc_error; };
waitUntil { sleep 2; _unitLeader distance _pos < ( _range ) };
if ( T8U_var_DEBUG ) then {[ ">> %1 >>>>>>>>>> loiter.sqf >> Unit is in Range of LoiterPos!! >> %2", time, _group ] call BIS_fnc_error; };
...
if ( count _seats > 0 ) then {
	...
	if ( T8U_var_DEBUG ) then { [ ">> %1 >>>>>>>>>> loiter.sqf >> SEATS >> %2", time, _seats ] call BIS_fnc_error; };
	...
	_n = 1;
	{
		...
		_units = _units - [ _x ];
		if ( T8U_var_DEBUG ) then { [ ">> %1 >>>>>>>>>> loiter.sqf >> %2 attached to %3", time, _x, _seat ] call BIS_fnc_error; };
		sleep 3;
...
_n = 1;
{
	...
	_units = _units - [ _x ];
	 if ( T8U_var_DEBUG ) then { [ ">> %1 >>>>>>>>>> loiter.sqf >> %2 attached to %3", time, _x, _chatterPosEP ] call BIS_fnc_error; };
	sleep 3;
...
if ( T8U_var_DEBUG ) then { [ ">> %1 >>>>>>>>>> loiter.sqf >> %2 >> Successfully Initialized", time, _group ] call BIS_fnc_error; };
...
if ( T8U_var_DEBUG ) then { [ ">> %1 >>>>>>>>>> loiter.sqf >> %2 >> TERMINATING", time, _group ] call BIS_fnc_error; };
