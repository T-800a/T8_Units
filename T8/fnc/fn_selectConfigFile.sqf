...
{
	case ( _CFalloc AND _CFMalloc ):	{ missionConfigFile >> "cfgT8Units";	__DEBUG( __FILE__, "CONFIG FROM", missionConfigFile ); };
	case ( _CFalloc AND !_CFMalloc ):	{ configFile >> "cfgT8Units";			__DEBUG( __FILE__, "CONFIG FROM", configFile );	};
	case ( !_CFalloc AND _CFMalloc ):	{ missionConfigFile >> "cfgT8Units";	__DEBUG( __FILE__, "CONFIG FROM", missionConfigFile ); };
	default								{ configNull };
};
