...
if ( ( ( typeName _caller ) == ( typename objNull ) AND { isNull _caller } ) OR ( ( typeName _caller ) == ( typeName [] ) AND { count _caller < 2 } ) ) exitWith { if ( T8U_var_DEBUG ) then { [ "fn_HALO.sqf", "ERROR: _caller" ] spawn T8U_fnc_DebugLog; }; false };
if ( ( ( typeName _target ) == ( typename objNull ) AND { isNull _target } ) OR ( ( typeName _target ) == ( typeName [] ) AND { count _target < 2 } ) ) exitWith { if ( T8U_var_DEBUG ) then { [ "fn_HALO.sqf", "ERROR: _target" ] spawn T8U_fnc_DebugLog; };false };
if ( typeName _caller == typename [] AND { count _this < 4 } ) exitWith { if ( T8U_var_DEBUG ) then { [ "fn_HALO.sqf", "ERROR: _caller is PosArray, need Spawnside" ] spawn T8U_fnc_DebugLog; }; false };
if ( count _units <= 0 ) exitWith { if ( T8U_var_DEBUG ) then { [ "fn_HALO.sqf", "ERROR: _units" ] spawn T8U_fnc_DebugLog; }; false };
...
[ _group, [ ( _spawnPos select 0 ), ( _spawnPos select 1 ), 0 ], "MOVE", "AWARE", "", 75, "FULL", [ 10, 10, 10 ] ] call T8U_fnc_CreateWaypoint;
[ _group, [ ( _targetPos select 0 ), ( _targetPos select 1 ), 0 ], "SAD", "COMBAT", "", 50, "FULL"] call T8U_fnc_CreateWaypoint;
